...
class Typing_state:
    ...
    def input_evaluator(self, screen):
        ...
        while count_row <= len(file_map) - 1:
            ...
            while char_index < file_map[count_row]["length"]:
                screen.move(
                    count_row, char_index + file_map[count_row]["indentation"] * 8
                )
                ...
                if key_press == backspace_key_code:
                    if wrong_typed > 0:
                        ...
                        self.render_user_input(
                            file_map, code_typed, wrong_typed, screen
                        )
                    ...
                    key_press == file_map[count_row]["text"][char_index]
                    and wrong_typed == 0
                ):
    ...
    def render_user_input(self, file_map, input_tree, wrong_typed, screen):
        ...
        for file_row, input_row in zip(file_map, input_tree):
            screen.addstr(
                count, file_row["indentation"] * 8 + len(file_row["text"]), "      "
            )
    ...
    def code_to_type_map(self, file):
        ...
        with open(file, "r") as fp:
            for count, line in enumerate(fp):
                ...
                lines.append(
                    {
                        "text": [char for char in list(line)],
                        "length": len(line),
